...
class PageDataBloc extends Bloc<PageDataEvent, PageDataState> {
  ...
  Stream<PageDataState> mapEventToState(
    ...
    if (event is PageDataEventForward) {
      final int pageNum =
          state.pageNum + 1 == state.pageTotal ? 0 : state.pageNum + 1;
      add(PageDataEventUpdate(pageNum));
    }

    if (event is PageDataEventBack) {
      final int pageNum =
          state.pageNum - 1 < 0 ? state.pageTotal - 1 : state.pageNum - 1;
      add(PageDataEventUpdate(pageNum));
    }
    ...
    if (event is PageDataEventUpdate) {
      ...
      yield PageDataStateUpdate(state,
          pageModel: model, pageNum: event.pageNum, pageTotal: list.length);
    }
  }
